...
class BobClient(object):
    ...
    def send_command(self, command):
        if not self.is_connected():
            self.reconnect()
        if self.verbose:
            logging.debug("SND: {}".format(command))
        try:
    ...
    def _prepare_rgb_color(self, lightname, r, g, b):
        ...
        fg = g / 255.0
        fb = b / 255.0
        self.send_command("set light {} rgb {} {} {}".format(lightname, fr, fg,
                                                             fb))
...
class Light(object):
    ...
    def __str__(self):
        return "<Light name='{}' hscan='{}-{}' vscan='{}-{}' color='{}/{}/{}'>".format(
            self.name, self.hmin, self.hmax, self.vmin, self.vmax, self.r,
            self.g, self.b)
